...
RSpec.describe TasksController, type: :controller do
  ...
  let(:valid_attributes) {
    { name: 'Task 1', description: 'Task 1 description', due_date: Date.today, difficulty: 'medium', estimate: 10, user_id: @user.id }
  }

  let(:invalid_attributes) {
    { user_id: user.id, name: '', description: '', due_date: nil, difficulty: 'medium', estimate: 0}
  }

  before do
    @user = FactoryBot.create(:user) # or User.create!(...)
